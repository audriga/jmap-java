...
public class EmailAddressTokenizerTest {
    ...
    public void singleChar() {
        ...
        Assertions.assertArrayEquals(
                expected.toArray(new EmailAddressToken[0]), actual.toArray(new EmailAddressToken[0]));
    }
    ...
    public void whitespaces() {
        final String input = "     ";
        final Collection<EmailAddressToken> actual = EmailAddressTokenizer.tokenize(input);
        Assertions.assertArrayEquals(new EmailAddressToken[0], actual.toArray(new EmailAddressToken[0]));
    }

    @Test
    public void simpleStandAloneEmailAddress() {
        final String input = "test@example.com";
        final Collection<EmailAddressToken> expected = ImmutableList.of(new EmailAddressToken(
    ...
    public void singleEmailWithLabel() {
        ...
        final Collection<EmailAddressToken> expected = ImmutableList.of(new EmailAddressToken(
                0, 16, EmailAddress.builder().email("a@example.com").name("A").build()));
        final Collection<EmailAddressToken> actual = EmailAddressTokenizer.tokenize(input);
    ...
    public void singleEmailWithLabelExcessWhiteSpace() {
        final String input = " A   <a@example.com>";
        final Collection<EmailAddressToken> expected = ImmutableList.of(new EmailAddressToken(
                0, 19, EmailAddress.builder().email("a@example.com").name("A").build()));
        final Collection<EmailAddressToken> actual = EmailAddressTokenizer.tokenize(input);
